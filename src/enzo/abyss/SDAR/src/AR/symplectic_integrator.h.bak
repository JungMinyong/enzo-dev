...
/* // Eunwoo - multiple definiion error!
    ! print features
...
*/ // Eunwoo - multiple definition error!
...
//#ifdef AR_DEBUG
//                    auto& pos1= pj->getLeftMember()->Position;
//                    auto& pos2= pj->getRightMember()->Position;
...
//#endif
        ...
        */
        ...
        Float calcInvR(Tparticle& _p1, Tparticle& _p2) {
            Float dr[3] = {_p1.Position[0] - _p2.Position[0], 
                           _p1.Position[1] - _p2.Position[1], 
                           _p1.Position[2] - _p2.Position[2]};
...
//#endif
        ...
        */
        ...
        void updateBinaryVelIter(AR::BinaryTree<Tparticle>& _bin) {
            _bin.Velocity[0]= _bin.Velocity[1] = _bin.Velocity[2] = 0.0;
            Float mcm_inv = 1.0/_bin.Mass;
            for (int k=0; k<2; k++) {
                if (_bin.isMemberTree(k)) {
                    auto* bink = _bin.getMemberAsTree(k);
                    updateBinaryVelIter(*bink);
                    _bin.Velocity[0] += bink->Mass*bink->Velocity[0];
                    _bin.Velocity[1] += bink->Mass*bink->Velocity[1];
                    _bin.Velocity[2] += bink->Mass*bink->Velocity[2];
                }
                else {
                    auto* pk = _bin.getMember(k);
                    _bin.Velocity[0] += pk->Mass*pk->Velocity[0];
                    _bin.Velocity[1] += pk->Mass*pk->Velocity[1];
                    _bin.Velocity[2] += pk->Mass*pk->Velocity[2];
                }
            }
            _bin.Velocity[0] *= mcm_inv;
            _bin.Velocity[1] *= mcm_inv;
            _bin.Velocity[2] *= mcm_inv;
        }

        //! update binary cm iteratively
        void updateBinaryCMIter(AR::BinaryTree<Tparticle>& _bin) {
            _bin.Position[0]= _bin.Position[1] = _bin.Position[2] = 0.0;
            _bin.Velocity[0]= _bin.Velocity[1] = _bin.Velocity[2] = 0.0;
            ...
            for (int k=0; k<2; k++) {
                if (_bin.isMemberTree(k)) {
                    ...
                    _bin.Position[0] += bink->Mass*bink->Position[0];
                    _bin.Position[1] += bink->Mass*bink->Position[1];
                    _bin.Position[2] += bink->Mass*bink->Position[2];
                    _bin.Velocity[0] += bink->Mass*bink->Velocity[0];
                    _bin.Velocity[1] += bink->Mass*bink->Velocity[1];
                    _bin.Velocity[2] += bink->Mass*bink->Velocity[2];
                }
                    ...
                    auto* pk = _bin.getMember(k);
                    mcm_member  += pk->Mass;
                    _bin.Position[0] += pk->Mass*pk->Position[0];
                    _bin.Position[1] += pk->Mass*pk->Position[1];
                    _bin.Position[2] += pk->Mass*pk->Position[2];
                    _bin.Velocity[0] += pk->Mass*pk->Velocity[0];
                    _bin.Velocity[1] += pk->Mass*pk->Velocity[1];
                    _bin.Velocity[2] += pk->Mass*pk->Velocity[2];
                }
            ...
            _bin.Velocity[1] *= mcm_inv;
            _bin.Velocity[2] *= mcm_inv;
            _bin.Position[0] *= mcm_inv;
            _bin.Position[1] *= mcm_inv;
            _bin.Position[2] *= mcm_inv;
        }
...
#elif  defined(AR_SLOWDOWN_ARRAY) // Eunwoo editted
        //! update slowdown factor based on perturbation and record slowdown energy change
        ...
        void initialIntegration(const Float _time) {
            ...
            if(particles.isOriginFrame()) {
                ...
                for (int i=0; i<info.binarytree.getSize(); i++) {
                    ...
                    bin.Position[0] -= particles.cm.Position[0];
                    bin.Position[1] -= particles.cm.Position[1];
                    bin.Position[2] -= particles.cm.Position[2];
                    bin.Velocity[0] -= particles.cm.Velocity[0];
                    bin.Velocity[1] -= particles.cm.Velocity[1];
...
*/
            ...
            while(true) {
                ...
                auto& bin_root = info.getBinaryTreeRoot();
                // auto& G = manager->interaction.gravitational_constant; // Original but compile error
                // double G = (double)(1.0); // Eunwoo added
                ...
                if(backup_flag) {
                    ...
                    if (manager->interrupt_detection_option>0 && !time_end_flag) {
                        ...
                        if (bin_interrupt.status!=InterruptStatus::none) {
                            ...
                            if (manager->interrupt_detection_option==2) {
                                ...
                                // info.generateBinaryTree(particles, G); // Original code
                                info.generateBinaryTree(particles, (double)1.0); // Eunwoo added
                                ...
                                // info.ds = info.calcDsKeplerBinaryTree(*bin_interrupt.adr, manager->step.getOrder(), G, manager->ds_scale); // Original
                                info.ds = info.calcDsKeplerBinaryTree(*bin_interrupt.adr, manager->step.getOrder(), (double)1.0, manager->ds_scale); // Eunwoo added
                    ...
                    if (!time_end_flag&&!binary_update_flag) {
                        // bool update_flag=updateBinarySemiEccPeriodIter(bin_root, G, time_); // Original
                        bool update_flag=updateBinarySemiEccPeriodIter(bin_root, (double)1.0, time_); // Eunwoo added
...
//#endif
        ...
        void correctCenterOfMassDrift() {
            ...
            for (int i=0; i<particles.getSize(); i++) {
                const Float *ri = particle_data[i].Position;
            ...
            for (int i=0; i<particles.getSize(); i++) {
                Float *ri = particle_data[i].Position;
        ...
        void writeBackSlowDownParticles(const Tptcl& _particle_cm) {
            ...
            for (int i=0; i<particles.getSize(); i++) {
                ...
                particle_adr[i]->Position[0] = particle_data[i].Position[0] + _particle_cm.Position[0];
                particle_adr[i]->Position[1] = particle_data[i].Position[1] + _particle_cm.Position[1];
                particle_adr[i]->Position[2] = particle_data[i].Position[2] + _particle_cm.Position[2];

                particle_adr[i]->Velocity[0] = particle_data[i].Velocity[0]*kappa_inv + _particle_cm.Velocity[0];
                particle_adr[i]->Velocity[1] = particle_data[i].Velocity[1]*kappa_inv + _particle_cm.Velocity[1];
                particle_adr[i]->Velocity[2] = particle_data[i].Velocity[2]*kappa_inv + _particle_cm.Velocity[2];
            }
            ...
            for (int i=1; i<nsd; i++) {
                ...
                for (int k=0; k<2; k++) {
                    ...
                    Float vrel[3] = { vel[0] - velcm[0], 
                                      vel[1] - velcm[1], 
                                      vel[2] - velcm[2]}; 
                    particle_adr[j]->Velocity[0] += vrel[0] * kappa_inv_m_one;
                    particle_adr[j]->Velocity[1] += vrel[1] * kappa_inv_m_one;
                    particle_adr[j]->Velocity[2] += vrel[2] * kappa_inv_m_one;
                }
            }
        }
#endif
        ...
        void writeBackSlowDownParticlesIter(const Tptcl& _particle_cm, const Float* _vel_sd_up, const Float& _inv_nest_sd_up, AR::BinaryTree<Tparticle>& _bin) {
            ...
            for (int k=0; k<2; k++) {
                if (_bin.isMemberTree(k)) {
                    ...
                    pk_adr->Position[0] = pk.Position[0] + _particle_cm.Position[0];
                    pk_adr->Position[1] = pk.Position[1] + _particle_cm.Position[1];
                    pk_adr->Position[2] = pk.Position[2] + _particle_cm.Position[2];

                    pk_adr->Velocity[0] = vel_sd[0] + _particle_cm.Velocity[0];
                    pk_adr->Velocity[1] = vel_sd[1] + _particle_cm.Velocity[1];
                    pk_adr->Velocity[2] = vel_sd[2] + _particle_cm.Velocity[2];
                }
            }
        }
...
//#endif
        ...
        */
        ...
        void printGroupInfo(const int _type, std::ostream& _fout, const int _width, const Tptcl* _pcm=NULL) {
            ...
            if (_pcm!=NULL) {
                pos_cm[0] = pcm_loc.Position[0] + _pcm->Position[0];
                pos_cm[1] = pcm_loc.Position[1] + _pcm->Position[1];
                pos_cm[2] = pcm_loc.Position[2] + _pcm->Position[2];
                vel_cm[0] = pcm_loc.Velocity[0] + _pcm->Velocity[0];
                vel_cm[1] = pcm_loc.Velocity[1] + _pcm->Velocity[1];
                vel_cm[2] = pcm_loc.Velocity[2] + _pcm->Velocity[2];
            }
            else {
                pos_cm[0] = pcm_loc.Position[0]; 
                pos_cm[1] = pcm_loc.Position[1]; 
                pos_cm[2] = pcm_loc.Position[2]; 
                vel_cm[0] = pcm_loc.Velocity[0]; 
                vel_cm[1] = pcm_loc.Velocity[1]; 
                vel_cm[2] = pcm_loc.Velocity[2]; 
            }
#pragma omp critical
            {
