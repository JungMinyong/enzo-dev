    ...
    public:
        ...
        */
        ...
        static void orbitToParticle(Tptcl& _p1, Tptcl& _p2, const Binary& _bin, const Float& _ecca, const Float _G) {
            ...
            _p1.Position[0] = -m2_mt * pos_red.x;
            _p1.Position[1] = -m2_mt * pos_red.y;
            _p1.Position[2] = -m2_mt * pos_red.z;

            _p2.Position[0] =  m1_mt * pos_red.x;
        ...
        */
        ...
        static void particleToOrbit(Binary& _bin, const Tptcl& _p1, const Tptcl& _p2, const Float _G){
            ...
            Vector3<Float> pos_red(_p2.Position[0] - _p1.Position[0], _p2.Position[1] - _p1.Position[1], _p2.Position[2] - _p1.Position[2]);
            Vector3<Float> vel_red(_p2.Velocity[0] - _p1.Velocity[0], _p2.Velocity[1] - _p1.Velocity[1], _p2.Velocity[2] - _p1.Velocity[2]);
            Float r_sq = pos_red * pos_red;
            _bin.r = sqrt(r_sq);
            Float inv_dr = 1.0 / _bin.r;
            Float v_sq = vel_red * vel_red;
            _bin.semi = 1.0 / (2.0*inv_dr - v_sq /  Gm_tot);
            // fprintf(stderr, "_p1.PID: %d, _p2.PID: %d\n", _p1.PID, _p2.PID); // Eunwoo debug
        ...
        static void calcMinDisList(int _ptcl_list[], 
            ...
            for (int i=0; i<_n_members-1; i++) {
                ...
                for(int j=i+1; j<_n_members; j++) {
                    ...
                    Vector3<Float> dr = {_ptcl[k].Position[0] - _ptcl[kj].Position[0],
                                         _ptcl[k].Position[1] - _ptcl[kj].Position[1],
                                         _ptcl[k].Position[2] - _ptcl[kj].Position[2]};
                    Float r2 = dr*dr;
                    if(r2<r2min) {
                        r2min = r2;
                        jc = j;
    ...
    public:
        ...
        */
        ...
        void calcCenterOfMass() {
            ...
            this->Position[0] = m1*member[0]->Position[0] + m2*member[1]->Position[0];
            this->Position[1] = m1*member[0]->Position[1] + m2*member[1]->Position[1];
            this->Position[2] = m1*member[0]->Position[2] + m2*member[1]->Position[2];
            this->Velocity[0] = m1*member[0]->Velocity[0] + m2*member[1]->Velocity[0];
            this->Velocity[1] = m1*member[0]->Velocity[1] + m2*member[1]->Velocity[1];
            this->Velocity[2] = m1*member[0]->Velocity[2] + m2*member[1]->Velocity[2];
            this->Mass = m1+m2;
            this->Position[0] /=this->Mass;
            this->Position[1] /=this->Mass;
            this->Position[2] /=this->Mass;
            this->Velocity[0] /=this->Mass;
            this->Velocity[1] /=this->Mass;
            this->Velocity[2] /=this->Mass;
        }
