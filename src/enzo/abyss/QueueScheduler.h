...
public:
    ...
    Worker* waitQueue(int type) {
        ...
        if (type == 0) // blocking
        {
            MPI_Probe(MPI_ANY_SOURCE, MPI_ANY_TAG, abyss_comm, &_status);
            _completed_queues++;
        ...
        {
            MPI_Iprobe(MPI_ANY_SOURCE, MPI_ANY_TAG, abyss_comm, &_flag, &_status);
            if (_flag) 
            {
                _rank = _status.MPI_SOURCE;
...
//private:
    ...
    void _checkCompletion(int &return_value) {
        MPI_Irecv(&return_value, 1, MPI_INT, MPI_ANY_SOURCE, TERMINATE_TAG, abyss_comm, &_request);
    }
