...
#pragma unroll Dim
    ...
    if (NumberOfSingleParticle != 0) {
        fprintf(nbpout, "NumberOfSingleParticle > 0\n");
...
int ReceiveParticleFromEnzo() {
    ...
    if (NumberOfSingleParticleOld != NumberOfSingleParticle) {
        ...
        std::unordered_map<int, int> PIDtoIndexMapCopy = PIDtoIndexMap;
        fprintf(stderr, "(before) ABYSS: NumberOfSingleParticle in Abyss: %d, NumberOfSingleParticle in Enzo: %d, newNumberOfSingleParticle: %d, PIDtoIndexMapCopy.size: %d\n",
            NumberOfSingleParticle, NumberOfSingleParticleOld, newNumberOfSingleParticle, PIDtoIndexMapCopy.size());

        for (int j = 0; j < NumberOfSingleParticleOld; j++) {
            PIDtoIndexMapCopy.erase(recvbuf_old[j].ID);
            fprintf(stderr, "%d, ", recvbuf_old[j].ID);
        }
        assert(PIDtoIndexMapCopy.size() == NumberOfSingleParticle - NumberOfSingleParticleOld);
        for (auto it = PIDtoIndexMapCopy.begin(); it != PIDtoIndexMapCopy.end(); ++it) {
            Particle* ptcl = &particles[it->second];
            if (ptcl->isActive == false) {
                // break CM particle first
                ptcl->CMPtclIndex = -1;
            }
        ...
        fprintf(stderr, "\n");
        fprintf(stderr, "(after) ABYSS: NumberOfSingleParticle in Abyss: %d, NumberOfSingleParticle in Enzo: %d, newNumberOfSingleParticle: %d\n",
    ...
    for (int i = 0; i <= global_variable->LastParticleIndex; i++) {
        ...
        if (!ptcl->isActive) {
            ...
            if (ptcl->CMPtclIndex != -1) {
                ...
                // These are those removed in Enzo (Minyong Jung)
                // Do not update NumberOfEscapeParticle
            }
