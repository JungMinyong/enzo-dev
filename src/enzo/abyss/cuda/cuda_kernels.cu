...
__global__ void compute_forces_init01(const CUDA_REAL* __restrict__ ptcl, const CUDA_REAL* __restrict__ r2, CUDA_REAL* __restrict__ diff, int m, int n, const int* __restrict__ subset, int* __restrict__ neighbor, int* num_neighbor, int i_start, int j_start, int NNB){
	// define i and j. in this code, grid is 2D and block is 1D
    int i = threadIdx.x + blockIdx.x * blockDim.x; // Unique thread index across all blocks
	int tid = threadIdx.x;
	// int BatchSize = blockDim.x;
	...
	while (i < m + BatchSize){ // even with i > m, the last block needs to assign the shared memory for each tid
		...
		if (i < m){
			...
			diff[idx_save + 6 * idx_save_size] = ax_irr;
			diff[idx_save + 7 * idx_save_size] = ay_irr;
			diff[idx_save + 8 * idx_save_size] = az_irr;
			diff[idx_save + 9 * idx_save_size] = jx_irr;
			diff[idx_save + 10 * idx_save_size] = jy_irr;
			diff[idx_save + 11 * idx_save_size] = jz_irr;
			num_neighbor[idx_save] = NumNeighbor;
		}
		i += gridDim.x * blockDim.x;
	} //end of i loop
...
// (EW to MY) please check diff part. Is that correct?
__global__ void compute_forces_init23(const CUDA_REAL* __restrict__ ptcl, const CUDA_REAL* __restrict__ r2, CUDA_REAL* __restrict__ diff, int m, int n, const int* __restrict__ subset, int* __restrict__ neighbor, int* num_neighbor, int i_start, int j_start, int NNB){
	// define i and j. in this code, grid is 2D and block is 1D
    int i = threadIdx.x + blockIdx.x * blockDim.x; // Unique thread index across all blocks
	int tid = threadIdx.x;
	// int BatchSize = blockDim.x;
	...
	while (i < m + BatchSize){ // even with i > m, the last block needs to assign the shared memory for each tid
			...
			for (int jj=0; jj<current_batch_size; jj++){
				...
				if (i<m){
					...
					if (magnitude0 > i_r2) { // regular force
						...
						CUDA_REAL rdf_r2 = ((dx * (a1x - a2x)) + (dy * (a1y - a2y)) + (dz * (a1z - a2z))) / magnitude0;
						CUDA_REAL vdf_r2 = ((dvx * (a1x - a2x)) + (dvy * (a1y - a2y)) + (dvz * (a1z - a2z))) / magnitude0;
						CUDA_REAL rdfdot_r2 = ((dx * (a1dotx - a2dotx)) + (dy * (a1doty - a2doty)) + (dz * (a1dotz - a2dotz))) / magnitude0;

						CUDA_REAL a = vr / magnitude0;
						CUDA_REAL b = v2 / magnitude0 + rdf_r2 + a*a;
						CUDA_REAL c = 3*vdf_r2 + rdfdot_r2 + a*(3*b-4*a*a);

						CUDA_REAL adot2x = - m_r3 * (a1x-a2x) - 6 * a * a21dotx - 3 * b * a21x;
						CUDA_REAL adot2y = - m_r3 * (a1y-a2y) - 6 * a * a21doty - 3 * b * a21y;
						CUDA_REAL adot2z = - m_r3 * (a1z-a2z) - 6 * a * a21dotz - 3 * b * a21z;
						
						ax_reg_dd += adot2x
						ax_reg_dd += adot2y
						ax_reg_dd += adot2z
						ax_reg_ddd += - m_r3 * (a1dotx-a2dotx) - 9 * a * adot2x - 9 * b * a21dotx - 3 * c * a21x;
						ax_reg_ddd += - m_r3 * (a1doty-a2doty) - 9 * a * adot2y - 9 * b * a21doty - 3 * c * a21y;
						ax_reg_ddd += - m_r3 * (a1dotz-a2dotz) - 9 * a * adot2z - 9 * b * a21dotz - 3 * c * a21z;
		...
		if (i < m){
			...
			diff[idx_save + 6 * idx_save_size] = ax_irr_dd;
			diff[idx_save + 7 * idx_save_size] = ay_irr_dd;
			diff[idx_save + 8 * idx_save_size] = az_irr_dd;
			diff[idx_save + 9 * idx_save_size] = ax_irr_ddd;
			diff[idx_save + 10 * idx_save_size] = ay_irr_ddd;
			diff[idx_save + 11 * idx_save_size] = az_irr_ddd;
			num_neighbor[idx_save] = NumNeighbor;
		}
		i += gridDim.x * blockDim.x;
	} //end of i loop
