...
#ifndef FEWBODY
		if (EPS2 > 0) r2 += EPS2;
#endif																													 // add the contribution of jth particle to acceleration of current and predicted times
...
void Particle::computeAccelerationReg() {
	...
	if (dt == 0.0) {
		dt  = this->TimeBlockReg*global_variable->time_step*global_variable->EnzoTimeStep; // interval of time step
		assert(dt != 0.0);
	}
...
void Particle::updateRegularParticleCuda() {
	...
	if (dt == 0.0) {
		dt  = this->TimeBlockReg*global_variable->time_step*global_variable->EnzoTimeStep; // unit conversion
		assert(dt != 0.0);
	}
	double dt2 = dt*dt;
	double dt3 = dt2*dt;
...
void Particle::initializeAfterCommunication() {
	...
	for (int i = 0; i < this->NewNumberOfNeighbor; i++) {
		...
		ptcl_neighbor = &particles[NewNeighbors[this->NeighborsOffset + i]];
		
		if (this->PID == ptcl_neighbor->PID)
			continue;
		...
		if (!ptcl_neighbor->isCMptcl) {
			Neighbors[this->NeighborsOffset + this->NumberOfNeighbor] = ptcl_neighbor->ParticleIndex;
			this->NumberOfNeighbor++;
		}
		else {
			Particle* ptcl_mem;
			for (int j = 0; j < ptcl_neighbor->NumberOfMember; j++) {
				ptcl_mem = &particles[ptcl_neighbor->Members[j]];
				Neighbors[this->NeighborsOffset + this->NumberOfNeighbor] = ptcl_mem->ParticleIndex;
				this->NumberOfNeighbor++;
			}
		}
		...
		if (this->TimeStepIrr == 0.0) {
			...
			if (sqrt(r2)*global_variable->a_i < RSEARCH/position_unit) // not considering vx < 0 because this primordial binary search is for avoiding too small time step case
				this->NewMembers[this->NewNumberOfMember++] = ptcl_neighbor->ParticleIndex;
#else
	...
	assert(this->NumberOfNeighbor >= this->NewNumberOfNeighbor - 1);
