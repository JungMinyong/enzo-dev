...
double ClusterRadius2; //EnzoCurrentTime, 
...
int InitialCommunication() {
...
#ifdef SEVN
	int *PID_SEVN;
	double *InitialMass_SEVN, *CreationTime_SEVN, *Metallicity_SEVN;
#endif

	MPI_Request request;
	MPI_Status status;

	fprintf(stderr, "NBODY+: First Waiting for Enzo to receive data...\n");

	CommunicationInterBarrier();
	fprintf(stderr, "NBODY+: Frist Receiving data from Enzo...\n");
	MPI_Recv(&NumberOfSingleParticle, 1, MPI_INT, 0, 100, inter_comm, &status);
	fprintf(stderr, "NBODY+: NumberOfSingleParticle=%d\n", NumberOfSingleParticle);
	if (NumberOfSingleParticle != 0) {
		PID           = new int[NumberOfSingleParticle];
		Mass          = new double[NumberOfSingleParticle];
		InitialMass   = new double[NumberOfSingleParticle];
	...
	fprintf(stderr, "data receiving!\n");


	MPI_Recv(&ComovingCoordinates        , 1, MPI_INT   , 0, 3000, inter_comm, &status);
	fprintf(stderr, "ComovingCoordinates=%d\n",ComovingCoordinates);
	if (ComovingCoordinates) {
		fprintf(stderr, "Cosmo data receiving!\n");
	...
	fprintf(stderr, "NumberOfEnzoSEVNParticle=%d\n", NumberOfEnzoSEVNParticle);
	if (NumberOfEnzoSEVNParticle != 0) {
		PID_SEVN			= new int[NumberOfEnzoSEVNParticle];
		InitialMass_SEVN	= new double[NumberOfEnzoSEVNParticle];
		CreationTime_SEVN 	= new double[NumberOfEnzoSEVNParticle];
		Metallicity_SEVN  	= new double[NumberOfEnzoSEVNParticle];
		MPI_Recv(PID_SEVN           , NumberOfEnzoSEVNParticle, MPI_INT   , 0, 4100, inter_comm, &status);
		MPI_Recv(InitialMass_SEVN   , NumberOfEnzoSEVNParticle, MPI_DOUBLE, 0, 4200, inter_comm, &status);
		MPI_Recv(CreationTime_SEVN  , NumberOfEnzoSEVNParticle, MPI_DOUBLE, 0, 4300, inter_comm, &status);
		MPI_Recv(Metallicity_SEVN   , NumberOfEnzoSEVNParticle, MPI_DOUBLE, 0, 4400, inter_comm, &status);
	}
#endif
	CommunicationInterBarrier();
	...
	fprintf(nbpout, "Nbody Time [Myr]              = %lf\n", EnzoCurrentTime*1e4);
	...
	*/
	...
	fprintf(stderr, "NBODY+: %d particles loaded!\n", NumberOfSingleParticle);
	// fflush(stdout);
	// fflush(stderr);
	fflush(stderr);
	//fflush(gpuout);
...
int ReceiveFromEnzo() {
...
#ifdef SEVN
	int *PID_SEVN, *newPID_SEVN;
	double *newMass_SEVN, *newMetallicity_SEVN;
#endif
	...
	if (NumberOfEnzoSEVNParticle != 0) {
		PID_SEVN = new int[NumberOfEnzoSEVNParticle];
		MPI_Recv(PID_SEVN , NumberOfEnzoSEVNParticle, MPI_INT   , 0, 2100, inter_comm, &status);
	}

	MPI_Recv(&newNumberOfEnzoSEVNParticle, 1, MPI_INT, 0, 2200, inter_comm, &status);
	if (newNumberOfEnzoSEVNParticle > 0)
	{
		newPID_SEVN           = new int[newNumberOfEnzoSEVNParticle];
		newMass_SEVN          = new double[newNumberOfEnzoSEVNParticle];
		newMetallicity_SEVN   = new double[newNumberOfEnzoSEVNParticle];
		MPI_Recv(newPID_SEVN          , newNumberOfEnzoSEVNParticle, MPI_INT   , 0, 2300, inter_comm, &status);
		MPI_Recv(newMass_SEVN         , newNumberOfEnzoSEVNParticle, MPI_DOUBLE, 0, 2400, inter_comm, &status);
		MPI_Recv(newMetallicity_SEVN  , newNumberOfEnzoSEVNParticle, MPI_DOUBLE, 0, 2500, inter_comm, &status);
	}
#endif
	CommunicationInterBarrier();
...
*/
	...
	if (newNumberOfEnzoSEVNParticle != 0) {
		delete[] newPID_SEVN;
		delete[] newMass_SEVN;
		delete[] newMetallicity_SEVN;
	}
	NumberOfEnzoSEVNParticle += newNumberOfEnzoSEVNParticle;
#endif
...
#ifdef SEVN
	assert(SEVNList_Enzo.size() == PIDtoIndexMap_SEVN.size());
	fprintf(stderr, "NBODY+    : original NumberOfEnzoSEVNParticle      = %d (+%d)\n", NumberOfEnzoSEVNParticle-newNumberOfEnzoSEVNParticle, newNumberOfEnzoSEVNParticle);
	fprintf(stderr, "NBODY+    : newly updated NumberOfEnzoSEVNParticle = %d\n", NumberOfEnzoSEVNParticle);
#endif
...
// (Query) We need to update FEWBODY routines incorporating hubble flows 
int SendToEnzo(Worker *workers) { 
	...
	double *Mass_Enzo, *newMass_Enzo;												// We need to update mass here
#endif
	double *HubbleFlow[Dim];
	...
	if (NumberOfEnzoSEVNParticle-newNumberOfEnzoSEVNParticle != 0) {
		WindEjectedMass_Enzo = new double[NumberOfEnzoSEVNParticle-newNumberOfEnzoSEVNParticle];
		SNEjectedMass_Enzo   = new double[NumberOfEnzoSEVNParticle-newNumberOfEnzoSEVNParticle];
		Temperature_Enzo     = new double[NumberOfEnzoSEVNParticle-newNumberOfEnzoSEVNParticle];

		Mass_Enzo 			= new double[NumberOfEnzoSEVNParticle-newNumberOfEnzoSEVNParticle];
	}
	if (newNumberOfEnzoSEVNParticle != 0) {
		newWindEjectedMass_Enzo = new double[newNumberOfEnzoSEVNParticle];
		newSNEjectedMass_Enzo   = new double[newNumberOfEnzoSEVNParticle];
		newTemperature_Enzo     = new double[newNumberOfEnzoSEVNParticle];

		newMass_Enzo 			= new double[newNumberOfEnzoSEVNParticle];
	}

	StellarEvolution_Enzo(Mass_Enzo, WindEjectedMass_Enzo, SNEjectedMass_Enzo, Temperature_Enzo,
							newMass_Enzo, newWindEjectedMass_Enzo, newSNEjectedMass_Enzo, newTemperature_Enzo);
#endif
...
#define noUPDATE_InitialNeighborRadius2
	...
	for (int dim = 0; dim < Dim; dim++)
	{
		...
		// (1 + RedshiftEnzoStart) / (1 + RedshiftEnzoEnd);
...
#ifdef SEVN
				if (ptcl->StellarEvolution != nullptr) {
					fprintf(stderr, "In SendToEnzo... PID: %d. SEVN memory is transferred to SEVNList_Enzo!\n", ptcl->PID);
	...
	if (newNumberOfSingleParticle > 0) {
		...
		for (int i=0; i<newNumberOfSingleParticle; i++) {
			...
			if (!ptcl->isActive) {
				...
				// 	newTemperature[i] *= -1; // This particle will be set to BlackHole in Enzo by EW 2025.6.25
...
#endif // END HUBBLD_FLOW
#endif // END COM_EVOLUTION
	}

#ifdef SEVN
	if (NumberOfEnzoSEVNParticle - newNumberOfEnzoSEVNParticle != 0) {
		MPI_Send(WindEjectedMass_Enzo, NumberOfEnzoSEVNParticle - newNumberOfEnzoSEVNParticle, MPI_DOUBLE, 0, 2000, inter_comm);
		MPI_Send(SNEjectedMass_Enzo,   NumberOfEnzoSEVNParticle - newNumberOfEnzoSEVNParticle, MPI_DOUBLE, 0, 2100, inter_comm);
		MPI_Send(Temperature_Enzo,     NumberOfEnzoSEVNParticle - newNumberOfEnzoSEVNParticle, MPI_DOUBLE, 0, 2200, inter_comm);
		MPI_Send(Mass_Enzo,           NumberOfEnzoSEVNParticle - newNumberOfEnzoSEVNParticle, MPI_DOUBLE, 0, 2300, inter_comm);
	}
	if (newNumberOfEnzoSEVNParticle != 0) {
		MPI_Send(newWindEjectedMass_Enzo, newNumberOfEnzoSEVNParticle, MPI_DOUBLE, 0, 2400, inter_comm);
		MPI_Send(newSNEjectedMass_Enzo,   newNumberOfEnzoSEVNParticle, MPI_DOUBLE, 0, 2500, inter_comm);
		MPI_Send(newTemperature_Enzo,     newNumberOfEnzoSEVNParticle, MPI_DOUBLE, 0, 2600, inter_comm);
		MPI_Send(newMass_Enzo,           newNumberOfEnzoSEVNParticle, MPI_DOUBLE, 0, 2700, inter_comm);
	}
#endif
	...
	if (NumberOfEnzoSEVNParticle-newNumberOfEnzoSEVNParticle != 0) {
		...
		delete[] SNEjectedMass_Enzo;
		delete[] Temperature_Enzo;
		delete[] Mass_Enzo;
	}
	if (newNumberOfEnzoSEVNParticle != 0) {
		delete[] newWindEjectedMass_Enzo;
		delete[] newSNEjectedMass_Enzo;
		delete[] newTemperature_Enzo;
		delete[] newMass_Enzo;
	}
#endif
