...
#ifdef NBODY
int grid::CopyActiveZonesFromGridNoStar(grid *OtherGrid, FLOAT EdgeOffset[MAX_DIMENSION],
    int SendField)
{
  ...
  while (true) 
    {
      ...
      if (CommunicationDirection == COMMUNICATION_POST_RECEIVE &&
        ...
        CommunicationReceiveGridOne[CommunicationReceiveIndex]  = this;
        CommunicationReceiveGridTwo[CommunicationReceiveIndex]  = OtherGrid;
        CommunicationReceiveCallType[CommunicationReceiveIndex] = 121;
        for (dim = 0; dim < GridRank; dim++)
          CommunicationReceiveArgument[dim][CommunicationReceiveIndex] = 
            EdgeOffset[dim];
        }
	...
	return SUCCESS;
			...
			if (SendField == GRAVITATING_MASS_FIELD_NO_STAR) {
        FORTRAN_NAME(copy3drel)(OtherGrid->GravitatingMassFieldNoStar,
                    GravitatingMassFieldNoStar,
                    Dim, Dim+1, Dim+2,
                    OtherDim, OtherDim+1, OtherDim+2,
                    GravitatingMassFieldDimension,
                    GravitatingMassFieldDimension+1,
