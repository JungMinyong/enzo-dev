...
int cons_plm(float **prim, float **priml, float **primr, int ActiveSize, int Neq, char direc)
{
  ...
  // limiting certain slopes 3.14 
    for (int i = 0; i < ActiveSize+1; i++) {
      iprim = i + NumberOfGhostZones - 1;
      // density
      dv[0][i] = sign(dv[0][i])*enzo_min(fabs(dv[0][i]), prim[0][iprim]/2);
      // pressure
      dv[1][i] = sign(dv[1][i])*enzo_min(fabs(dv[1][i]), prim[1][iprim]/(1.+Gamma));
      // velocities
      for (int field = 2; field < 5; field++) 
	dv[field][i] = sign(dv[field][i])*enzo_min(fabs(dv[field][i]), 2./prim[9][iprim]) ;
    }
