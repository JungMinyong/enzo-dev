  ...
  // Reference: Colella & Woodward (1984), Marti & Muller (1996)  
{
  ...
  for (int i = 0; i < ActiveSize+1; i++) {
    ...
    for (int field = 0; field < Neq; field++) {
      ...
      if (1) {
	// 1) Contact steeping
	if (Gamma*K0*fabs(rho_p1-rho_m1)/MIN_VAL(rho_p1,rho_m1) >= 
	    fabs(p_p1-p_m1)/MIN_VAL(p_p1, p_m1)) {
	  ...
	  d2am1 = a - 2.0*am1 + am2;
	  ap1mam1 = ap1 - am1;
	  if (-d2ap1*d2am1 > 0 && fabs(ap1mam1) - eps1*MIN_VAL(fabs(ap1),fabs(am1)) > 0) {
	    eta0 = -(d2a-d2am1)/(6.0*ap1mam1);
	  } else {
	    eta0 = 0.0;
	  }
	  
	  eta = MAX_VAL(0.0, MIN_VAL(eta1*(eta0-eta2), 1.0));
	...
	// 2) Shock flattening
	if (i >= 1 && i <= ActiveSize) {
	  if (fabs(p_p1-p_m1)/MIN_VAL(p_p1,p_m1) > eps2 && v_m1 > v_p1) {
	    f0 = MIN_VAL(1.0, MAX_VAL(0.0, ((p_p1-p_m1)/(p_p2-p_m2) - w1)*w2));
	    if (p_p1 - p_m1 > 0) {
	      p_p3 = prim[ipres][iprim+3];
	      f0p1 = MIN_VAL(1.0, MAX_VAL(0.0, ((p_p2-p)/(p_p3-p_m1) - w1)*w2));
	      f = MAX_VAL(f0, f0p1);
	    } else {
	      p_m3 = prim[ipres][iprim-3];
	      f0m1 = MIN_VAL(1.0, MAX_VAL(0.0, ((p-p_m2)/(p_p1-p_m3) - w1)*w2));
	      f = MAX_VAL(f0, f0m1);
	    }
