...
int DepositParticleMassFieldNoStar(HierarchyEntry *Grid, FLOAT Time = -1.0);

int CommunicationBufferPurge(void);
int CommunicationReceiveHandler(fluxes **SubgridFluxesEstimate[] = NULL,
		int NumberOfSubgrids[] = NULL,
		int FluxFlag = FALSE,
		TopGridData* MetaData = NULL,bool NoStar = NOSTAR_YES);
...
#endif  // end FAST_SIB
{
	...
	if (traceMPI) 
		fprintf(tracePtr, "PrepareDensityFieldNoStar: Enter DepositParticleMassField (Send)\n");
	...
	for (StartGrid = 0; StartGrid < NumberOfGrids; StartGrid += GRIDS_PER_LOOP) {
		...
		for (grid1 = StartGrid; grid1 < EndGrid; grid1++)
			DepositParticleMassFieldNoStar(Grids[grid1], EvaluateTime);
		...
		if (debug1) fprintf(stdout,"NoStar 4-3\n");  // by YS
	...
	for (StartGrid = 0; StartGrid < NumberOfGrids; StartGrid += GRIDS_PER_LOOP) {
		...
		if (debug1) fprintf(stdout,"4-4\n");  // by YS
		for (grid1 = StartGrid; grid1 < EndGrid; grid1++)
			PrepareGravitatingMassField1(Grids[grid1]);

		/* Next, send data and process grids on the same processor. */

		if (debug1) fprintf(stdout,"4-5\n");  // by YS
		CommunicationDirection = COMMUNICATION_SEND;
		for (grid1 = StartGrid; grid1 < EndGrid; grid1++)
			PrepareGravitatingMassField1(Grids[grid1]);
