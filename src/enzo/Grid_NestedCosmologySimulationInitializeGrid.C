    ...
    if (IndividualStarPopIIIFormation){
      ...
      if (IndividualStarPopIIISeparateYields){
        for (int yield_i = 0; yield_i < StellarYieldsNumberOfSpecies; yield_i++){
          if(StellarYieldsAtomicNumbers[yield_i] > 2){
            int field_num = 0;
            this->IdentifyChemicalTracerSpeciesFieldsByNumber(field_num, StellarYieldsAtomicNumbers[yield_i],0,2);
  ...
  if (MultiMetals) {
	...
	*/
	...
	if (CosmologySimulationGasEnergyName == NULL && DualEnergyFormalism)
	  ...
	  // Below lines was not presented in the Grid_CosmologySimulationInitializeGrid.C
      // Shock/Cosmic Ray Model
      if (ShockMethod && ReadData) {
	for (i = 0; i < size; i++) {
	  BaryonField[MachNum][i] = tiny_number;
