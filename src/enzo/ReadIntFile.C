...
int ReadIntFile(char *name, int Rank, int Dim[], int StartIndex[],
  ...
  if (Rank == 3)
	...
	for (i = StartIndex[0]; i <= EndIndex[0]; i++)
	  buffer[k*Dim[0]*Dim[1] + j*Dim[0] + i] =
	    (int) ((*tempbuffer)[(k-StartIndex[2])*TempIntArray[0]*TempIntArray[1] +
