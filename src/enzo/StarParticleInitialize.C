...
#define no_aeos_debug
    ...
    InitializeStellarYields(
        MetaData->Time); // by YS Should be turned on!! just a test for now
#ifdef aeos_debug
  ...
  if (SkipFeedbackFlag) {
    ...
    for (cstar = AllStars; cstar; cstar = cstar->NextStar) {
      ...
      if (STARMAKE_METHOD(INDIVIDUAL_STAR))
        cstar->AssertInterpolationPositions(); // should be set at init, but
                                               // double check
  ...
  //  MetaData->NumberOfParticles); fprintf(stdout, "MetaData->NumberOfParticles
  //  = %d\n", MetaData->NumberOfParticles); fprintf(stdout,
  //  "NumberOfStarParticles now = %d\n", NumberOfStarParticles);
