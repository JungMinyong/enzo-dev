...
*/
  ...
  for (dim = 0; dim < MAX_DIMENSION; dim++) {
    ...
    if (Layout[dim] == 0)
      Layout[dim] = MAX_VAL(nint((float)(Dims[dim])/Edge), 1);
  ...
  if (Grid->ParentGrid != NULL) {
	...
	if (CoarseSlab < NumberOfCoarseSlabs+FirstCoarseSlab &&
	  ...
	  ThisEndIndex = MIN_VAL(CoarseEdges[CoarseSlab], Dims[dim]);
	else
	  ThisEndIndex = Dims[dim];
	ThisStartIndex = MAX_VAL(CoarseEdges[CoarseSlab-1], 0);
	...
	NumberOfSlabs = MAX_VAL( nint( float(ThisWidth) / ExactDimsLeft ), 1);
	...
	if (CoarseLimit > 0 && ThisEndIndex != Dims[dim])
	  NumberOfSlabs = MIN_VAL(NumberOfSlabs, CoarseLimit);
  ...
  for (dim = 0; dim < MAX_DIMENSION; dim++) {
    MPI_Bcast(GridDims[dim], Layout[dim], DataTypeInt, ROOT_PROCESSOR, enzo_comm);
    MPI_Bcast(StartIndex[dim], Layout[dim], DataTypeInt, ROOT_PROCESSOR, enzo_comm);
  }
#endif  
